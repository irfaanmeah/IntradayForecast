...
####--------------------------------------------Forecast Setup-------------------------------##
path1="C:/Users/irfaa/Google Drive (irfaan.meah@gmail.com)/Work/GitHub Repo/IntradayForecast/"

setwd(path1)

#Source functions and script
...
shinyApp(
  ...
  server = function(input, output,session) {
    ...
    observeEvent(input$save,{
      ...
      temp <- data.frame("Received.Date"=as.POSIXct(paste0(input$date," 00:00:00",format="%Y-%m-%d",tz="")),
                         "interval"=input$interval_choices,
                         "Actual"=input$act_inc_vol,
                         "backlog"=input$act_backlog,check.names = FALSE)
      #print(temp)
      # temp <- data.frame("Received.Date"=as.POSIXct("2018-02-01",format="%Y-%m-%d",tz=""),"interval"="08:15",
      #                    "Actual"=20,
      #                    "backlog"=10,check.names = FALSE)

      input_df <-bind_rows(values$input_df,temp)
      
      input_df$Received.Time <- as.POSIXct(paste0(date(input_df$Received.Date)," ",input_df$interval),tz = "")
