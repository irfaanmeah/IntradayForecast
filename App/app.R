...
load(paste0(path,"pre_load_files_for_app.RData"))
...
#input_df <- input_df[1:(nrow(input_df)-1),]
...
final_df <- readRDS(file = paste0(path,"final_df.rds"))

#temp
# final_df$forecast[1] <- 8
# final_df$Actual[1] <- NA
# final_df$backlog[1] <- NA 
##
...
for(i in 1:nrow(final_df)){
    ...
    final_df$interval[i] <- ifelse(nchar(substr(final_df$interval[i],gregexpr(":",final_df$interval[i])[[1]][1]+1,
                                                nchar(final_df$interval[i])))<2,paste0(final_df$interval[i],"0"),
                                   final_df$interval[i])
    
    final_df$interval[i] <- ifelse(nchar(substr(final_df$interval[i],1,gregexpr(":",final_df$interval[i])[[1]][1]-1))<2,
                                   paste0("0",final_df$interval[i]),
                                   final_df$interval[i])
        ...
        #mainPanel(
            #tabsetPanel(
                ...
                #tabPanel("Intraday Forecast",
                         ...
                         dateInput("date", label = h3("Date input"), value = "2018-01-31"),
    ...
    server = function(input, output,session) {
        ...
        output$plot1 <- renderPlotly({
            ...
            plot_ly(values$final_df, x = ~interval, y = ~Actual, name = 'Actuals', type = 'scatter', mode = 'lines') %>%
                ...
                layout(xaxis = x, yaxis = y)#,yaxis2 = ay
        ...
        observeEvent(input$save,{
           ...
           # )
            ...
            temp <- data.frame("Received.Date"=as.Date(input$date,format="%Y-%m-%d",""),
                               "interval"=input$interval_choices,
                               "Actual"=input$act_inc_vol,
                               "backlog"=input$act_backlog,check.names = FALSE)
            #print(temp)
            # temp <- data.frame("Received.Date"=as.Date("2018-01-31"),"interval"="08:15",
            #                    "Actual"=16,
            ...
            ##-------------------------------------Forecast Horizon---------------------------------##
                ...
                if(nrow(horizon_df)==freq){
                    ...
                    horizon_df_base <- readRDS(file = paste0(path,"horizon_base.rds"))
                ...
                max_date <- max(values$input_df$Received.Date)
                
                input_df_all_choices <- unique(values$input_df$interval)
                input_df_choice <- values$input_df[values$input_df$Received.Date==max_date,"interval"]
        ...
        observeEvent(input$save_cp,{
            ...
            total_volume <- sum(c(values$last_act_backlog,values$remaining_forecast),na.rm = T)
